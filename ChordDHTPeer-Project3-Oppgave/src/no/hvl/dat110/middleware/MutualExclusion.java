...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		boolean premission = areAllMessagesReturned(peers.size());
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				int timeA = message.getClock();
				
				// own clock for the multicast message
				int timeB = clock.getClock();
				
				// compare clocks, the lowest wins
				if (timeA < timeB || (timeA == timeB && message.getNodeID().compareTo(node.getNodeID()) < 0)) {
					NodeInterface stub = Util.getProcessStub(procName, port);
					message.setAcknowledged(true);
					stub.onMutexAcknowledgementReceived(message);
				} else {
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		boolean test = queueack.size() == numvoters;
